...
from src.database import Saver
...
class VKinder:
    ...
    def search_users(self, age: int, gender: int, city: int, status: int, count: int = 1000) -> list:
        """
        ...
        Args:
            ...
            count (int):  Количество пользователей (по умолчанию 1000)
...
class UserDataCache:
    ...
    def initialize_user_data(self, user_id: int, connector) -> None:
        ...
        """
        self.cache[user_id] = {"step": None, "in_db": connector.get_user_data_from_db(user_id)}
    ...
    def get_next_profile(self, user_id: int) -> dict | None:
        ...
        Returns:
            dict or None: Информация об анкете или None, если анкеты закончились.
        """
        if not self.cache[user_id].get('profiles'):
            return None
        if user_id in self.cache and "profiles" in self.cache[user_id]:
            return self.cache[user_id]["profiles"].pop(-1)
        return None
...
class VkBot:
    ...
    def send_photos_and_link(self, user_id: int, photos: list[dict], link: str) -> None:
        """
        Отправляет фотографии и ссылку на пользователя ВКонтакте.

        Args:
            user_id (int):       Идентификатор пользователя, которому отправлять сообщение.
            photos (List[dict]): Список фотографий.
            link (str):          Ссылка на пользователя ВКонтакте.
        """
        attachments = ",".join(
            [f"photo{photo['owner_id']}_{photo['id']}" for photo in photos]
        )
        self.api.messages.send(
            user_id=user_id, attachment=attachments, message=link, random_id=0
        )
    ...
    def process_final(self, user_id: int, *_) -> str:
        """
        ...
        if "profiles" in data:
            profiles_id = [
                profile["id"] for profile in data["profiles"]
            ]
            ...
            self.worker_cache.save_user_data(user_id, "in_db", data["in_db"])
        self.send_message(user_id, messages.PROCESS_FINAL)
        return "final"
    ...
    def process_message(self, event) -> None:
        ...
        if current_data is None:
            ...
            self.send_message(event.user_id, messages.GREET_FIRST)
            current_step = None
        else:
    ...
    def handle_final_step(self, user_id: int, text: str, _) -> str:
        """
        Обработка конечного состояния, когда найдены пользователи
        ...
        Returns:
            str: Следующий шаг
        """
        if text.lower() == messages.NEXT_PEOPLE.lower():
            next_profile = self.worker_cache.get_next_profile(user_id)
            if next_profile:
                top_photos = self.vkinder.get_top_photos(next_profile["id"])
                self.send_photos_and_link(
                    user_id, top_photos, f"https://vk.com/id{next_profile['id']}"
                )
                return "final"
            self.send_message(user_id, messages.NO_MORE_PROFILES)
            return "again"
        if text.lower() == messages.AGAIN_SEARCH.lower():
            self.send_message(
                user_id, "\n".join([messages.TRY_AGAIN, messages.PROCESS_AGE])
            )
            return "age"
        self.send_message(user_id, messages.ERROR_OTHER)
        return "final"

    def handle_search_users(self, user_id: int, text: str, current_step: str) -> str:
        """
        ...
        if self.is_valid_input(text, "status"):
            ...
            token = VK_USER_TOKEN
            if not token:
                self.send_message(user_id, messages.ERROR_SESSION)
                return current_step
            ...
            if next_profile:
                ...
                self.send_message(user_id, messages.PROCESS_FINAL)
            return self.process_final(user_id)
        self.send_message(user_id, messages.ERROR_MESSAGE_DATA)
        return current_step
    ...
    def is_valid_input(text: str, step: str) -> bool:
        ...
        valid_inputs = {
            ...
            "final": lambda text: text.lower() in [messages.NEXT_PEOPLE.lower(),
                                                   messages.AGAIN_SEARCH.lower()],
